...
export const createPaypalOrder = action({
  ...
  handler: async (
    ...
    try {
      ...
      const paymentId = await ctx.runMutation(api.payments.createPayment, {
        userId,
        amount: amount,
        credits: credits,
  ...
  handler: async (ctx, { orderID, userId }) => {
    try {
      ...
      if (capture.status === "DECLINED" || capture.status === "FAILED") {
        if (paymentId) {
          await ctx.runMutation(api.payments.updatePayment, {
            paymentId,
      ...
      //add new credit
      await ctx.runMutation(api.creditManagement.addCredits, {
        userId,
